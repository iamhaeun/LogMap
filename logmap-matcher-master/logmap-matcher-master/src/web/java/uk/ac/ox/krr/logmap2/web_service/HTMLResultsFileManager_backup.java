 ...
 *
 */
public class HTMLResultsFileManager_backup {
	...
	public HTMLResultsFileManager duplicate() {
		return new HTMLResultsFileManager(this.file_output, this.uri_output, this.relative_output_path,
				this.output_folder, this.uri_folder, this.uri1, this.uri2);
	}
	...
	public HTMLResultsFileManager_backup(String output_folder, String uri_folder, String uri1, String uri2) {
	...
	// To clone class
	public HTMLResultsFileManager_backup(String T_file_output, String T_uri_output, String T_relative_output_path,
			String output_folder, String uri_folder, String uri1, String uri2) {
	...
	public HTMLResultsFileManager_backup(String base_path, String base_uri, String name, String version, String uri1,
			String uri2) {
		...
		this.date4folder = day + "_" + month + "_" + year + "__" + hour + "_" + minute + "_" + second + "_"
				+ milisecond;
	...
	private void writeBodyHTMLFile() {
		...
		writer.writeLine("<p><input type=\"button\" value=\"Go to request form\" onClick=\"window.location.href='"
				+ base_uri + "'\"></p>");

		writer.writeLine("<fieldset>");
		writer.writeLine("<p><b>Progress of the matching task on " + date2show + " by " + name + " using " + version
		...
		writer.writeLine("<li><b>Ontology 1:</b> " + "<a href=\"" + uri1 + "\" target=\"_blank\" id=\"onto1\">" + uri1
				+ "</a></li>");
		writer.writeLine("<li><b>Ontology 2:</b> " + "<a href=\"" + uri2 + "\" target=\"_blank\" id=\"onto2\">" + uri2
				+ "</a></li>");
	...
	private void addHeaderMappings2Ask_Form(int showing, int num_mappinsg2ask) {
		...
		try {
			...
			writer.writeLine("<fieldset>");
			writer.writeLine("<legend><b>Mappings requiring user feedback</b> (assessing " + showing + " out of "
					+ num_mappinsg2ask + " remaining mappings)</legend>");

			// writer.writeLine("<p><b>Remaining mappings to assess: " + num_mappinsg2ask +
			// "</b></p>");
	...
	private void addTailMappings2Ask_Form() {
		...
		// writer.writeLine("<button id=\"end_interaction\">Finish
		// interaction</button>");
		// writer.writeLine("<button id=\"save_session\">Save session</button>");
		writer.writeLine("</td>");
		writer.writeLine("</tr>");
	 ...
	 */
	private void addMapping2Form(int id_mapping, // identifier mapping (its order in structure)
		...
		writer.writeLine("<a onclick =\"javascript:ShowHide('DivURIs" + id_mapping
		...
		if (mappingsInConflict.size() > 0) {
			...
			writer.writeLine("<br />");
			writer.writeLine(
					"<label>If the current mapping is ADDED the following mappings will be DISCARDED.</label>");
			writer.writeLine("<br />");
			writer.writeLine("<br />");
		...
		if (mappingsAmbiguous.size() > 0) {
			...
			writer.writeLine("<br />");
			writer.writeLine(
					"<label>If the AMBIGUITY CRITERIA is used and the current mapping is ADDED (respectively DISCARDED) the following mappings will be DISCARDED (respectively ADDED).</label>");
