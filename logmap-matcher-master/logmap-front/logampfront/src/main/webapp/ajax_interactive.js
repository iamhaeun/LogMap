...
function init() {
  ...
  if (ajax4) {
	...
	//Note that when we will remove the dic we will also remove the buttons
      ...
      document.getElementById('start_interaction').onclick=buttonSendFeedbackAction(ajax4, ''); //buttons 
...
//-------------------------
function buttonSendFeedbackAction(ajax4, param) {
	...
	if(param=='asc' || param=='desc'){
		sortVal = param;
	}else if(param.indexOf('search')!= -1){
		searchVal = param.substr(7);
	}else{
	...
	ajax4.open("POST", "http://localhost:8080/logmap/interactivity.do", true);
//	ajax4.open("POST", "../../interactivity.do", true);
	...
	ajax4.send("folder="+folder+"&feedback="+feedback+"&ontouri1="+ontouri1+"&ontouri2="+ontouri2+"&sort="+sortVal+"&search="+searchVal);
...
///////////////////////////////************* */
	...
	if (results.innerHTML.indexOf("The interactivity process has finished.")!=-1){
		//end of process
		document.getElementById('start_interaction').style.display='none';
...
function SortingStatus(sort){
  ...
  if (ajax4) {
    // Check for DOM support:
    if (document.getElementById('mappings')) {  //Div for outputs
      var a = buttonSendFeedbackAction(ajax4, sort); 
      a();
    }
  }
...
function MappingStatus(radioId) {

	if (radioId.includes('add')) {
		num_mapping = radioId.replace('add', 'bg');
		if (document.getElementById(num_mapping).classList.contains('list-group-item-primary') == false) {
			document.getElementById(num_mapping).classList.replace('list-group-item-danger', 'list-group-item-primary');
		}
	} else {
		num_mapping = radioId.replace('del', 'bg');
		if (document.getElementById(num_mapping).classList.contains('list-group-item-danger') == false) {
			document.getElementById(num_mapping).classList.replace('list-group-item-primary', 'list-group-item-danger');
		}
...
function SearchingFunc() {
	...
	// Get an XMLHttpRequest object:
	var ajax4 = getXMLHttpRequestObject();
	var searchingVal = document.getElementById('searchInput').value;

	if (searchingVal == null || searchingVal == '') {
		alert("Please input text for searching");
	} else {
		if (ajax4) {
			// Check for DOM support:
			if (document.getElementById('mappings')) {  //Div for outputs
				var a = buttonSendFeedbackAction(ajax4, "search." + searchingVal);
				a();
			}
		}
	}
